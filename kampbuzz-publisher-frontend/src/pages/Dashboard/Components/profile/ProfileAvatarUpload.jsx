...
const publisherProfileAvatarUploadSchema = z.object({
...
const ProfileAvatarUpload = () => {
  ...
  const form = useForm({
    resolver: zodResolver(publisherProfileAvatarUploadSchema),
  });
  ...
  const submit = async (data) => {
    ...
    setUploading(true);
    try {
      const res = await crudServices.uploadPublisherAvatar(formData);
      ...
      <CardContent>
        <div className="grid gap-2 mt-2">
          {loading ? (
            ...
            <div className="flex justify-center items-center">
              <img
                alt={publisher?.name || "User Profile Image"}
                className="aspect-square rounded-full object-cover"
                src={publisher?.avatar || "https://via.placeholder.com/200"}
                height={200}
                width={200}
              />
            </div>
